...
export const Register: React.FC = () => {
  const { register: registerUser, isAuthenticated, user, loading, error, clearError } = useAuthStore();
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-950 relative overflow-hidden px-4 sm:px-6">
      ...
      <div className="w-full max-w-md bg-slate-900/75 border border-slate-800/80 rounded-3xl overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.4)] backdrop-blur-2xl animate-fade-in relative z-10">
        ...
        <div className="bg-gradient-to-tr from-brand-700/80 to-brand-500/80 p-8 text-center border-b border-slate-800/80">
          ...
          <p className="text-brand-200 text-xs mt-1 font-semibold">Register to schedule interviews and track active requests.</p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-8 space-y-6">
          ...
          <div>
            ...
            <div className="relative group">
              <User className="absolute left-3.5 top-3.5 w-4 h-4 text-slate-400 group-focus-within:text-brand-400 transition-colors" />
              <input
                type="text"
                {...register("name")}
            ...
            {errors.name && (
              <p className="text-rose-400 text-xs mt-1 font-medium">{errors.name.message as string}</p>
            )}
          </div>
          ...
          <div className="text-center pt-2">
            <p className="text-slate-500 text-xs font-semibold">
              Already registered?{" "}
              <Link to="/login" className="text-brand-400 hover:text-brand-300 font-bold transition-all">
                Sign In Instead
              </Link>
            </p>
          </div>
